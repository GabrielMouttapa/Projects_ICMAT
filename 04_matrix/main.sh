...
for n in $(seq 1 $i); do
    for m in $(seq 1 $j); do
        ...
        cp scr/write_matrix.* data/inputs/folder_${n}x${m};
...
qsub -q all.q -e "${location}/data/outputs/errors/del.err" -o "${location}/data/outputs/outs/del.out" ${location}/scr/del.sh ${location}/data/inputs
